...
wget https://github.com/thomaswcy/R2S/releases/download/$(date +%Y.%m.%d)-R2Swan&lan_stable/openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-ext4-sysupgrade.img.gz
if [ -f /tmp/openwrt-rockchip-armv8-friendlyarm_nanopi-r4s-ext4-sysupgrade.img.gz	]; then
	echo -e '\e[92m最新固件已下载，准备解压\e[0m'
else
	wget https://github.com/thomaswcy/R2S/releases/download/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y.%m.%d)-R2Swan&lan_stable/openwrt-rockchip-armv8-friendlyarm_nanopi-r2s-ext4-sysupgrade.img.gz
